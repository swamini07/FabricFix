...
import com.trial.Home.LocationAccess;
...
public class HomePage extends Application {
    ...
    public void start(Stage primaryStage) {
        this.primaryStage = primaryStage;
    Choice choice = new Choice(primaryStage, this::handleBackButton);
        ...
        Button locationBtn = new Button("📍 Allow Location");
        locationBtn.setOnAction(e -> locationAccess.requestLocation(primaryStage, locationBtn));

        ChoiceBox<String> loginChoice = new ChoiceBox<String>();
        loginChoice.getItems().addAll("New Customer ?", "Profile", "Orders", "Wishlist", "Rewards");
        loginChoice.getSelectionModel().selectedItemProperty().addListener((obs, oldVal, newVal) -> {
            ...
            if (newVal != null) {
                switch (newVal) {
                    case "New Customer ?":
                        ...
                        break;
                    case "Profile":
                        choice.openProfilePage();
                        break;
                    case "Orders":
                        choice.openOrdersPage();
                        break;
                    case "Wishlist":
                        choice.openWishlistPage();
                        break;
                    case "Rewards":
                        choice.openRewardsPage();
                        break;
                }
    ...
    private void initializeSignUp() {
        SignUp signUp = new SignUp();
        signUp.setHomePageStage(primaryStage);
